    ...
    FakeSidecar() {
        ...
        server_.Get("/zones", [](const httplib::Request&, httplib::Response& res) {
            res.set_content(
                json{
                    {"zones",
                     {{{"zone_id", "zone-1"}, {"display_name", "Main Room"}, {"state", "paused"}}}}}
                    .dump(),
                "application/json");
        });
...
int main() {
    try {
        ...
        require(status.now_playing->artist == "The Drivers",
                "status should parse now playing artist");
        require(status.now_playing->album == "Horizon Radio",
                "status should parse now playing album");
        require(status.now_playing->position_ms == 67000,
                "status should convert seek seconds to ms");
        require(status.now_playing->duration_ms == 245000,
                "status should convert length seconds to ms");
        ...
        require(cached->selected_zone_name == "Main Room",
                "cached status should keep selected zone name");
        ...
        FakeSidecar::status_ok      = false;
        const auto unhealthy_status = client.status();
        require(!unhealthy_status.ok, "status should preserve sidecar ok=false");
        ...
        std::cerr << "unknown roon_control_client_tests failure\n";
        return 1;
    }
}
